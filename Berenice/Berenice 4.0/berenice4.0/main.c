...
void exibir();
void salvar();
void salvar_txt_binario();
void submenu();
void ler();
void ler_txt_binario();
...
int main()
{
    ...
    do
    {
        printf("\n\n_______Mercadinho da Berenice_______\n");
        printf("            1 - Produtos              \n");
        printf("            2 - Vendas                \n");
        printf("            3 - Sair                  \n");
...
void submenu()
{
    ...
    while (escolha != 9)
    {
        printf("\n\n________________Produtos____________\n");
        ...
        printf("        6 -     Ler Dados             \n");
        printf("        7 -    Save Binario           \n");
        printf("        8 -    Ler Binario            \n");
        printf("        9 -       Sair                \n");
...
void submenu_venda()
{
    ...
    while(loop != 1)
    {
        printf("\n\n ______________Venda Berenice____________\n");
...
int escolha;
    printf("%d", contador_items);
    printf("\n Pressione 0 para voltar");
    printf("\n Pressione 1 para salvar em formato binario");
    printf("\n--> ");
    scanf("%d", &escolha);

    if (escolha == 1)
    {
        FILE *arquivo_b = fopen("BereBinario.bin", "wb");
        if (arquivo_b == NULL)
        {
        ...
        fwrite(&contador_items, sizeof(int), 1, arquivo_b);

        for (int i = 0; i < contador_items; i++)
        {
            fwrite(&estoque[i].codigo, sizeof(int), 1, arquivo_b);
            fwrite(estoque[i].nome, sizeof(char), MAX_nome, arquivo_b);
            fwrite(&estoque[i].preco, sizeof(float), 1, arquivo_b);
            fwrite(&estoque[i].quantidade, sizeof(int), 1, arquivo_b);
        }

        printf("\n\nItens arquivados em Formato binario com sucesso!");
        fclose(arquivo_b);
    }
...
void ler_txt_binario()
{
    FILE *arquivo_b = fopen("BereBinario.bin", "rb");
    ...
    fread(&contador_items, sizeof(int), 1, arquivo_b);

    estoque = (produto *)realloc(estoque, contador_items * sizeof(produto));

    for (int i = 0; i < contador_items; i++)
    {
        fread(&estoque[i].codigo, sizeof(int), 1, arquivo_b);
        fread(estoque[i].nome, sizeof(char), MAX_nome, arquivo_b);
        fread(&estoque[i].preco, sizeof(float), 1, arquivo_b);
        fread(&estoque[i].quantidade, sizeof(int), 1, arquivo_b);
    }

    printf("\n\nItens lidos em formato binario com sucesso!");
    fclose(arquivo_b);
    submenu();
}
...
void atualizar()
{
    ...
    {
        printf("\n\n_________Items Berenice__________");
        printf("\n|Codigo|Nome do Item|Valor|Estoque|\n");
        for(int i = 0; i<contador_items; i++)
        {
            printf("|%6d|%12s|%4.2f|%6d|\n", estoque[i].codigo, estoque[i].nome, estoque[i].preco,estoque[i].quantidade);
        ...
        {
            printf("\nCodigo Encontrado!");
            printf("\n\n________Items Berenice________");
            printf("\n|Codigo|Nome do Item|Valor|Estoque|\n");
            for(int i = 0; i<contador_items; i++)
            {
...
void venda()
{
    ...
    do{
        ...
        printf("\n\n___________MERCADO DA BERE___________");
        printf("\n|Codigo|Nome do Item|Valor|Estoque|\n");
        for(int i = 0; i<contador_items; i++)
        {
            printf("|%6d|%10s|%4.2f|%6d|\n", estoque[i].codigo, estoque[i].nome, estoque[i].preco,estoque[i].quantidade);
        ...
        estoque[validar].vendidos = estoque[validar].vendidos + qnt_venda;
        estoque[validar].vendidos_relatorio = estoque[validar].vendidos_relatorio + qnt_venda;
...
void relatorio()
{
    ...
    printf("\n\n__________Relatorio de Vendas__________");
...
void salvar_relatorio_txt()
{
    time_t t = time(NULL); //Pega informações de data e hora do sistema e armazena em t.
    struct tm tm = *localtime(&t); //Envia t para uma estrutura struct(tm)
    char nome_arquivo[50]; //Cria o nome do arquivo, utilizando as respectivas funções da biblioteca <time.h>
    snprintf(nome_arquivo, sizeof(nome_arquivo), "%d-%02d-%02d_%02d-%02d-%02d.txt",
    ...
    if(arquivo == NULL)
    {
        printf("Nao foi possivel abrir o arquivo %s",nome_arquivo);
        return;
    }
...
void cupom(float valor_t)
{
     printf("\n\n__________CUPOM FISCAL__________");
